...
const props = defineProps<{
  data: { platform: string, totalSpend: number, totalClicks: number }[]
}>()

const x = (_: unknown, i: number) => i
const y = (d: { totalSpend: number }) => d.totalSpend

const tickFormat = (i: number) => props.data[i]?.platform ?? ''
</script>

<template>
  <UCard>
    <template #header>
      <h3 class="font-semibold">По площадкам</h3>
    </template>
    ...
    <VisXYContainer v-else :data="data" :height="300" :padding="{ top: 10, right: 10, bottom: 30, left: 60 }">
      <VisGroupedBar :x="x" :y="[y]" color="#22d3ee" :bar-padding="0.3" :rounded-corners="4" />
